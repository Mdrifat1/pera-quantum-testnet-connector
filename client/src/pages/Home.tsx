...
type AutoRequestStatus = "scheduled" | "awaiting-approval" | "submitted" | "rejected" | "failed" | "stopped";
type AutoRequest = { requestId: string; runId: string; walletIndex: number; address: string; signer: string; txn: Transaction; amountMicro: number; recipient: string; status: AutoRequestStatus; createdAt: number; dispatchedAt: number; settledAt?: number; txid?: string; error?: string };
...
export default function Home() {
  ...
  const dispatchAutoRequest = async (runId: string) => {
    if (!autoRequestRef.current || runIdRef.current !== runId || inFlightRequestsRef.current.size >= MAX_IN_FLIGHT_SIGNING) return;
    ...
    try {
      ...
      const txn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({ sender: address, receiver: recipient.trim(), amount: amountMicro, suggestedParams, note: new TextEncoder().encode(`Pera Quantum ${network} approval demo`) });
      const request: AutoRequest = { requestId: `${runId}-${autoRequestsUsedRef.current}`, runId, walletIndex, address, signer: address, txn, amountMicro, recipient: recipient.trim(), status: "awaiting-approval", createdAt: Date.now(), dispatchedAt: Date.now() };
      ...
      pushActivity(makeActivity("Submitted", `${formatAlgo(request.amountMicro)} ALGO · ${txid.slice(0, 12)}…`, "success"));
    } catch (error) {
      const message = String(error).toLowerCase();
      settleAutoRequest({ ...(inFlightRequestsRef.current.get(`${runId}-${autoRequestsUsedRef.current}`) ?? { requestId: `${runId}-${autoRequestsUsedRef.current}`, runId, walletIndex, address, signer: address, txn: {} as Transaction, amountMicro: 0, recipient: recipient.trim(), status: "awaiting-approval" as AutoRequestStatus, createdAt: Date.now(), dispatchedAt: Date.now() }) }, message.includes("reject") || message.includes("cancel") || message.includes("close") ? "rejected" : "failed", { error: String(error) });
    }
  };
